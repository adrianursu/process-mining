...
def create_event_element(attributes):
    ...
    for key, value in attributes.items():
        if key == "time:timestamp":
            ET.SubElement(event_elem, "date", key=key, value=str(value))
        else:
...
def json_log_to_xes(json_data):
    ...
    with alive_bar(len(json_data), bar="bubbles", spinner="dots", title="Convert JSON to XES", 
        ...
        for round_data in json_data:
            ...
            if round_data["change_location_events"]:
                for change_event in round_data.get("change_location_events", []):
                    event_attrs = {
                        "concept:name": f"Move to {change_event['new_place']}",
                        "time:timestamp": change_event["timestamp"],
                        "org:role": change_event["player"],
                        "player_place": change_event["old_place"]
                    }
                    events.append((event_attrs["time:timestamp"], event_attrs))                
...
# Save to XES file
with open("game_round_log6.xes", "wb") as f:
